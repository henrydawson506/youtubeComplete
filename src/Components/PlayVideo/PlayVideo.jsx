...
async function fetchVideoData(params) {
    const videoDetailsUrl=`https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&id=${videoId}&key=${API_KEY}`;
    //2:11:58 url from api id= key=
    const data=await fetch(videoDetailsUrl);
...
async function fetchChannelData(){
    ...
    const res=await data.json();
     ...
     const commentData=await fetch(comment_url);
     const commentRes=await commentData.json();
...
<div className='play-video-info'>
    <p>{apiData?value_converter(apiData.statistics.viewCount):'views unavailable'} Views &bull;{ moment(apiData?apiData.snippet.publishedAt:'').fromNow()}</p> 
<div>
...
<div className='publisher'>
   ...
   { commentData.map((item,index)=>{
        ...
        <p>{item?item.snippet.topLevelComment.snippet.textDisplay:''}</p></div>
